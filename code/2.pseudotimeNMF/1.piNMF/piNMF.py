...
path.append('/home/jovyan/jm_jlab/data_CBL/indNeuro_tmp/2.pseudotimeNMF/1.piNMF')
from FunctionalHALS import fHALS
from SHALS import buildS 
...
# JM: MODIFIED FOR PSEUDOTIME-INFORMED NMF
    def _nmf(self, X, nmf_kwargs):
        ...
        X = pd.DataFrame(X)
        r= nmf_kwargs['n_components'] # factorization rank
        d=3 #degree of nonnegative B-Spline
        m=X.shape[0]
        n=X.shape[1]
        kn=np.linspace(np.min(X.index),np.max(X.index), 4) #Number of knots

        options = {'kn':kn, 'heuri':0, 'ftol':1e-10, 'maxiter':1e4}

        S = buildS(d=d,kn=kn, xp=np.linspace(kn[0],kn[-1],m))
              
        if 'random_state' in nmf_kwargs.keys(): #JM: Main computation 
            np.random.seed(nmf_kwargs['random_state'])
            (B,spectra,error,iter) = fHALS(X.to_numpy(), r=r, d=d, case='S', funtype='bs', initB=None, initX=None, options=options)
            usages = np.dot(S, B)
        else:            
            #JM: A last iteration initializing X with the consensus spectra to obtain consensus usages
            (B,spectra,error,iter) = fHALS(X.to_numpy(), r=r, d=d, case='S', funtype='bs', initB=None, initX=nmf_kwargs['H'], options=options)
            usages = np.dot(S, B)
    ...
    def consensus(self, k, density_threshold=0.5, local_neighborhood_size = 0.30,show_clustering = True,
        ...
        spectra_tpm = median_spectra #jm: no refitting
        #spectra_tpm = self.refit_spectra(tpm.X, norm_usages.astype(tpm.X.dtype)) #jm: no refitting
