---
title: "TF binding disruptions in regulatory islands"
...
__The aim of this notebook is to run motifbreakR on a selected list of regulatory regions__

__In This version of the notebook I have implemented the following changes to the analysis:__

* Only the top scoring motif for alleleDiff score is retained for each TF-SNPid association, dropping duplicates PWMs

* Scores are inversed in order to display negative scores for reduced affinity and positive scores for increased affinity to modern alleles
...
```{r drop variants w/o rsid, collapse=TRUE}
#rsids <- regions$rsID[!regions$rsID == 'Nan']
...
## Read only variants associated to a RSID

```{r read rsids, collapse=TRUE}
#SNPS.from.rsids <- snps.from.rsid(rsid = rsids,
...
ResDF_hg19_hocomoco$alleleDiff <- -ResDF_hg19_hocomoco$alleleDiff
```
...
* 3. Bind rows to regenerate comprehensive result table

```{r collapse=TRUE}
ResDF_all_hocomoco_pos <- ResDF_hg19_hocomoco %>% filter(alleleDiff > 0) #126,398 (75% quantile)
ResDF_all_hocomoco_neg <- ResDF_hg19_hocomoco %>% filter(alleleDiff < 0) #126,398 (25% quantile)
...
```{r Magnitude of effect hocomoco, fig.height=5, fig.width=9, collapse=TRUE,dpi=150}
data.frame(table(ResDF_hocomoco_topmotifs$effect)) %>% ggplot(., aes(x = Var1, y = Freq, fill = Var1)) +
  ...
  labs(x = '', y = 'Number of disruptions', fill = 'Magnitude of effect', title = 'hocomoco') +
...
Here I apply a filtering strategy by dropping everything out of the 4th quantile of the distribution 
...
**HOWEVER IN THIS NOTEBOOK WE HAVE COMPUTED THE OPPOSITE OF THE SCORES FOR A MORE INTUITIVE ASSOCIATION BETWEEN SCORES AND AFFINITY DIFFERENCE**
...
quantile(ResDF_hocomoco_topmotifs$alleleDiff)

ResDF_hocomoco_topmotifs_filt_pos <- ResDF_hocomoco_topmotifs_pos %>% filter(alleleDiff >= 1.1324056) #126,398 (75% quantile)
ResDF_hocomoco_topmotifs_filt_neg <- ResDF_hocomoco_topmotifs_neg %>% filter(alleleDiff <= -1.0379252) #126,398 (25% quantile)
```

* Total number of putative disruptions identified (pre-filtering): __`r nrow(ResDF_hocomoco_topmotifs)`__

* AlleleDiff distrubtion's quantiles: __`r quantile(ResDF_hocomoco_topmotifs$alleleDiff)`__

* Number of strong putative disruptions identified (post-filtering): __`r nrow(ResDF_hocomoco_topmotifs_filt_pos)`__

* Number of strong putative gains identified (post-filtering): __`r nrow(ResDF_hocomoco_topmotifs_filt_neg)`__


# Calculate pvalues

```{r collapse=TRUE}
tmp <- rbind(ResDF_hocomoco_topmotifs_filt_pos, ResDF_hocomoco_topmotifs_filt_neg)

pval.grange <- mbrkresults_regislands_hg19_hocomoco[mbrkresults_regislands_hg19_hocomoco$seqMatch %in% tmp$seqMatch, ] #extract entries from result object as GRanges format is expected from calcpvalues function

saveRDS(pval.grange, file = '/group/testa/Users/oliviero.leonardi/myProjects/EVAS/ContainerHome/RegIslands/Savings/GR_hocomoco_topmotifs_filt.posneg.rds')

#pval.grange <- readRDS(file='/group/testa/Users/oliviero.leonardi/myProjects/EVAS/ContainerHome/RegIslands/Savings/GR_hocomoco_topmotifs_filt.posneg_pvalues.rds')
```
...
quantile(data.frame(table(ResDF_hocomoco_topmotifs_filt_pos$geneSymbol) / table(ResDF_hocomoco_topmotifs[which(ResDF_hocomoco_topmotifs$geneSymbol %in% ResDF_hocomoco_topmotifs_filt_pos$geneSymbol),]$geneSymbol))$Freq)

selTF_hocomoco_pos <- data.frame(table(ResDF_hocomoco_topmotifs_filt_pos$geneSymbol) / table(ResDF_hocomoco_topmotifs[which(ResDF_hocomoco_topmotifs$geneSymbol %in% ResDF_hocomoco_topmotifs_filt_pos$geneSymbol),]$geneSymbol)) %>% filter(Freq >= 0.39208075)
```

```{r Ditribution quantiles for TF enriched for negative disruption over the total, collapse=TRUE}
quantile(data.frame(table(ResDF_hocomoco_topmotifs_filt_neg$geneSymbol) / table(ResDF_hocomoco_topmotifs[which(ResDF_hocomoco_topmotifs$geneSymbol %in% ResDF_hocomoco_topmotifs_filt_neg$geneSymbol),]$geneSymbol))$Freq)

selTF_hocomoco_neg <- data.frame(table(ResDF_hocomoco_topmotifs_filt_neg$geneSymbol) / table(ResDF_hocomoco_topmotifs[which(ResDF_hocomoco_topmotifs$geneSymbol %in% ResDF_hocomoco_topmotifs_filt_neg$geneSymbol),]$geneSymbol)) %>% filter(Freq >= 0.3846154)
```
...
totable <- ungroup(ResDF_hocomoco_topmotifs) #ungroup
totable <- totable[, -c(20,21)] #remove pvalue columns

write.table(totable, file=paste0(paperfolder, 'RegIslands.hocomoco.hg19.motifbreakR.topmotifs.unfiltered.tsv'), quote = FALSE, sep = '\t', row.names = F, col.names = TRUE)
```
